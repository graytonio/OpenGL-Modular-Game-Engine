...
public class MasterRenderer {
	...
	public MasterRenderer(Loader loader, float skyboxSize, Camera cam) {
		enableCulling();
		createProjectionMatrix();
		renderer = new EntityRenderer(shader, projectionMatrix);
		terrainRenderer = new TerrainRenderer(terrainShader, projectionMatrix);
		skyboxRenderer = new SkyboxRenderer(loader, projectionMatrix, skyboxSize);
		normalMapRenderer = new NormalMappingRenderer(projectionMatrix);
		shadowRenderer = new ShadowMapMasterRenderer(cam);
	}
	...
	public void render(List<Light> lights, Camera camera, Vector4f clipPlane) {
		...
		terrainShader.loadMapSize(Terrain.SIZE);
